...
#  ip_cleaned_all <- {};
 ...
 # }
  ...
  X_allapps_ip     <- cbind(X_allapps$AppDate, X_allapps$Sortcode,X_allapps$Age,X_allapps$IPAddresscleaned);
  X_whole_apps     <- cbind(seq(1,dim(as.matrix(X_allapps))[1],1),as.matrix(X_allapps[,12]));
